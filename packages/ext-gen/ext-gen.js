...
function stepStart() {
  ...
  var pkg = (fs.existsSync(nodeDir + '/package.json') &&
      JSON.parse(fs.readFileSync(nodeDir + '/package.json', 'utf-8')) || {});
  ...
  switch (mainCommand) {
    ...
    case 'app':
      ...
      const appCommandArgs = commandLineArgs(appDefinitions, {
        argv: appArgs, stopAtFirstUnknown: true
      });
      ...
      if (!appSubArgs.length) {
        ...
        try {
          cmdLine = commandLineArgs(optionDefinitions, {
            argv: appSubArgs, stopAtFirstUnknown: false
          });
        }
        catch (e) {
    ...
    case 'vp':
      ...
      const viewCommandArgs = commandLineArgs(viewDefinitions, {
        argv: viewArgs, stopAtFirstUnknown: true
      });
      var template = '';
      var name = viewCommandArgs.name;

      if (viewCommandArgs.template == null) {
        template = 'basic';
      }
      else {
...
function stepCheckCmdLine() {
  ...
  else if (cmdLine.defaults) {
    displayDefaults();
  }
  else if (cmdLine.command !== 'app') {
    console.log(`${app} ${boldRed('[ERR]')} unknown command '${cmdLine.command}'`);
  }
  else if (cmdLine.interactive && cmdLine.command === 'app') {
    stepSeeDefaults();
  }
  else if (process.argv.length === 2) {
    stepShortHelp();
  }
  else if (cmdLine.auto) {
    stepGo();
  }
  else if (cmdLine.name != null) {
    cmdLine.auto = true;
    stepGo();
  }
  else {
...
async function stepCreate() {
  ...
  try {
    const substrings = [
      '[ERR]', '[WRN]', '[INF] Processing', '[INF] Server',
      '[INF] Writing content', '[INF] Loading Build',
      '[INF] Waiting', '[LOG] Fashion waiting'
    ];
...
-v --verbose       verbose npm messages (for problems only)`;
    ...
    This template contains 1 profile, configured to use the classic toolkit of Ext JS for a
    desktop application.

    ${boldGreen('universalclassicmodern')}
    This template contains 2 profiles, 1 for desktop (using the classic toolkit), and 1 for
    mobile (using the modern toolkit).
